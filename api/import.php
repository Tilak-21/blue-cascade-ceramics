<?php
/**
 * Import all tiles data - run this once after deployment
 */
...
try {
    ...
    echo "<h2>🚀 Starting import of all 40 tiles...</h2>\n";
    ...
    foreach ($tilesData as $tile) {
        $searchTerms = strtolower($tile['series'] . ' ' . $tile['material'] . ' ' . $tile['category']);
    ...
    $totalInventory = array_sum(array_column($tilesData, 'qty'));
    $totalValue = array_sum(array_map(function($tile) { 
        return $tile['qty'] * $tile['proposedSP']; 
    }, $tilesData));
    
    // Log the import
    logAction('BULK_IMPORT', 'TILE', 'all-tiles', [
        'importCount' => $importCount,
        'totalInventory' => $totalInventory,
        'totalValue' => $totalValue
    ]);
    
    echo "<p>✅ Imported $importCount tiles</p>\n";
    echo "<p>📊 Total inventory: " . number_format($totalInventory) . " units</p>\n";
    echo "<p>💰 Total value: $" . number_format($totalValue, 2) . "</p>\n";
    echo "<h2>🎉 All 40 tiles imported successfully!</h2>\n";
    echo "<p><strong>Admin credentials:</strong></p>\n";
    echo "<ul><li>Username: admin</li><li>Password: CascadeTiles2024</li></ul>\n";
    echo "<p><a href='/'>← Back to website</a> | <a href='/admin'>Admin Panel</a></p>\n";
