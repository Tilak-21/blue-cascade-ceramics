// Complete import of ALL 40 tiles from frontend data
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

// All your original tile data - copied directly from tileData.js
const allTileData = [
  {
    type: 'GP',
    size: 'SL60X120TH9',
...
async function importAllTiles() {
  try {
    console.log(`🚀 Starting import of ALL ${allTileData.length} tiles...`);
      ...
      try {
        await prisma.tile.create({
          data: {
            ...
            description: `${tile.series} - ${tile.category} with ${tile.surface} finish. ${tile.thickness} thickness.`,
            isActive: true,
          },
        });
        
        importCount++;
        totalInventory += tile.qty;
        totalValue += (tile.qty * tile.proposedSP);
        
        if (importCount % 10 === 0) {
          console.log(`  ✅ Progress: ${importCount}/${allTileData.length} tiles imported...`);
        }
      } catch (error) {
        console.error(`❌ Error importing ${tile.series}:`, error.message);
      }
    }

    console.log(`\n🎉 IMPORT COMPLETED SUCCESSFULLY!`);
    console.log(`📊 Total tiles imported: ${importCount}`);
    console.log(`📦 Total inventory units: ${totalInventory.toLocaleString()}`);
    ...
    await prisma.auditLog.create({
      data: {
        action: 'FULL_IMPORT',
        entity: 'TILE',
        entityId: 'complete-catalog',
        adminId: null,
        changes: JSON.stringify({
          importCount,
          totalInventory,
          totalValue,
          source: 'frontend-tileData.js',
    ...
    console.log(`   • ${importCount} active tiles`);
    console.log(`   • ${totalInventory.toLocaleString()} total inventory`);
    console.log(`   • $${totalValue.toLocaleString()} inventory value`);
